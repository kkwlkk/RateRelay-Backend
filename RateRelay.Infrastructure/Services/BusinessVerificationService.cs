    ...
    : IBusinessVerificationService
{
    public async Task<BusinessVerificationResult> InitiateVerificationAsync(string placeId, long accountId)
    {
        ...
        var existingBusiness = (await businessRepository.FindAsync(
            b => b.PlaceId == placeId && b.OwnerAccountId == accountId)).FirstOrDefault();
        ...
        if (existingBusiness is { IsVerified: true })
        {
            logger.Warning("Business with place ID {PlaceId} is already verified by account {AccountId}",
                placeId, existingBusiness.OwnerAccountId);

            return BusinessVerificationResult.AlreadyVerified(existingBusiness);
        }
        ...
        {
            if (existingBusiness.OwnerAccountId != accountId)
            {
                logger.Warning("Business with place ID {PlaceId} is already being verified by another account",
                    placeId);
                return BusinessVerificationResult.AlreadyBeingVerified(existingBusiness);
            }
        ...
        var activeVerification = (await businessVerificationRepository
            .FindAsync(v => v.BusinessId == business.Id && v.VerificationCompletedUtc == null)).FirstOrDefault();

        if (activeVerification is not null)
        {
            // If verification is expired, create a new one
